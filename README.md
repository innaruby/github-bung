...
if in an Excel file , if it detect a sheet named Sachaufwand , it can written in small letter or capital letters , 
then look in the value in the column index A in that sheet starting from row 5 till the end row whether the value in that is found as a sheet Name. For example if the value in column 
index A is Balance , and if a sheet named Balance is present then go to that sheet , identify in which row of the column index A in that sheet the value Summe written in 
bold is found , that means along with the keyword Summe other words or numbers may also be present. Then identify in that row which all columns are visible starting from column index B till the last visible 
column which is second column containing the keyword Veränderung., then 
copy the all the values from each columns which are visible in that single row , and then copy that 
to the sheet named Sachaufwand corresponding to the row where it has the value Balance. Ist similar to v-look up. But in a different way. Similarly for the other values also. 
